...
func (e mapStateEntry) withListener(listener string, proxyPort, priority uint16) mapStateEntry {
	if proxyPort == 0 {
		listener = ""
		priority = 0
	} else if priority == 0 {
...
func TestMapState_denyPreferredInsertWithChanges(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			...
			ms: testMapState(map[Key]mapStateEntry{
				ingressKey(1, 3, 80, 0): allowEntry.withListener("listener1", 8080, 0),
			}),
			args: args{
				key: ingressKey(1, 3, 80, 0),
				entry: MapStateEntry{
			...
			want: testMapState(map[Key]mapStateEntry{
				ingressKey(1, 3, 80, 0): allowEntry.withListener("listener2", 9090, 1),
			}),
			wantAdds: Keys{
				ingressKey(1, 3, 80, 0): struct{}{},
			},
		...
		},
		{
			...
			ms: testMapState(map[Key]mapStateEntry{
				ingressKey(1, 3, 64, 10): allowEntry.withListener("listener1", 8080, 0),
			}),
			args: args{
			...
			want: testMapState(map[Key]mapStateEntry{
				ingressKey(1, 3, 64, 10): allowEntry.withListener("listener2", 9090, 1),
			}),
			wantAdds: Keys{
				ingressKey(1, 3, 64, 10): struct{}{},
			},
		...
		},
		{
			...
			want: testMapState(map[Key]mapStateEntry{
				ingressKey(1, 3, 80, 0): allowEntry.withListener("listener1", 8080, 1),
			}),
			wantAdds:    Keys{},
			wantDeletes: Keys{},
			wantOld: map[Key]mapStateEntry{
		...
		},
		{
			...
			want: testMapState(map[Key]mapStateEntry{
				ingressKey(1, 3, 64, 10): allowEntry.withListener("listener1", 8080, 1),
			}),
			wantAdds:    Keys{},
			wantDeletes: Keys{},
			wantOld: map[Key]mapStateEntry{
		...
		},
		{
			...
			ms: testMapState(map[Key]mapStateEntry{
				ingressKey(0, 3, 80, 0): allowEntry.withListener("", 8080, 0),
			}),
			args: args{
				key:   ingressKey(1, 3, 64, 10), // port range 64-127 (64/10)
				entry: MapStateEntry{},
